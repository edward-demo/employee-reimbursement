...
export function OpticalReimbursementForm({ onBack, onSubmit }: OpticalReimbursementFormProps) {
  ...
  const getTotalAmount = () => {
    const itemsTotal = opticalItems.reduce((total, item) => total + item.subtotal, 0);
    ...
    return itemsTotal - totalDeductions;
  };
  ...
  const isFormValid = () => {
    const opticalItemsValid = opticalItems.every(item => item.name && item.quantity && item.unitPrice);
    ...
    return opticalItemsValid && documentsValid && invoicesValid && isConfirmed;
  };
          ...
          <Card className="border-2 border-primary/10">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                <Label className="flex items-center opacity-72" style={{ color: 'rgba(0, 0, 0, 0.64)' }}>
                ...
                {prescriptionFiles.length < 3 && (
                  <div className="border-2 border-dashed border-primary/20 rounded-lg p-6 text-center hover:border-primary/40 transition-colors">
                    <div className="space-y-2">
                      ...
                      <p className="text-sm" style={{ color: 'rgba(0, 0, 0, 0.64)' }}>
                        Click to upload or drag and drop
                      </p>
                      <p className="text-xs" style={{ color: 'rgba(0, 0, 0, 0.64)' }}>
                      ...
                      <Card key={receipt.id} className="border-2 border-green-200 bg-green-50/50">
                        <CardContent className="p-4 space-y-3">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center space-x-2">
                              <div className="flex items-center space-x-2">
                                <Label htmlFor={`pwd-${receipt.id}`} className="text-sm" style={{ color: 'rgba(0, 0, 0, 0.64)' }}>PWD</Label>
                                <Switch
                                  id={`pwd-${receipt.id}`}
                          ...
                          {/* Invoice Number */}
                          <div className="space-y-2">
                            <Label className="text-sm opacity-72" style={{ color: 'rgba(0, 0, 0, 0.64)' }}>Invoice/Receipt Number *</Label>
                            {receipt.isEditingInvoice || !receipt.invoiceNumber ? (
                              <div className="flex space-x-2">
                                <Input
                                  ...
                                  className="border-primary/30 focus:border-primary placeholder:opacity-64"
                          ...
                          {/* Conditional VAT and PWD Discount Fields */}
                          {receipt.isPWD && (
                            <>
                              <div className="space-y-2">
                                <Label className="text-sm" style={{ color: 'rgba(0, 0, 0, 0.64)' }}>VAT Deduction (₱)</Label>
                                <Input
                                  placeholder="0.00"
                                  type="number"
                                  step="0.01"
                              ...
                              <div className="space-y-2">
                                <Label className="text-sm" style={{ color: 'rgba(0, 0, 0, 0.64)' }}>PWD Discount (₱)</Label>
                                <Input
                                  placeholder="0.00"
                                  type="number"
                                  step="0.01"
          ...
          <Card className="border-2 border-primary/10">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-4">
                {opticalItems.map((opticalItem, index) => (
                  <Card key={opticalItem.id} className="border border-primary/20 bg-primary/5">
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center space-x-2">
                          <Glasses className="h-4 w-4 text-primary" />
                          <h4 className="font-medium">Eye Care Item {index + 1}</h4>
                        </div>
                        {opticalItems.length > 1 && (
                          <Button
                            ...
                            variant="ghost"
                            size="sm"
                            onClick={() => removeOpticalItem(opticalItem.id)}
                            className="text-red-600 hover:text-red-700 hover:bg-red-50"
                          >
                            <Trash2 className="h-4 w-4" />
                          </Button>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="md:col-span-2 space-y-2">
                          <Label className="opacity-72" style={{ color: 'rgba(0, 0, 0, 0.64)' }}>Item Name *</Label>
                          <Input
                            placeholder="e.g., Eyeglasses, Contact Lenses"
                            value={opticalItem.name}
                            onChange={(e) => updateOpticalItem(opticalItem.id, 'name', e.target.value)}
                            className="border-primary/30 focus:border-primary placeholder:opacity-64"
                            style={{ backgroundColor: '#FFFFFF' }}
                          />
                        </div>
                        ...
                        <div className="space-y-2">
                          <Label className="opacity-72" style={{ color: 'rgba(0, 0, 0, 0.64)' }}>Quantity *</Label>
                          <Input
                            placeholder="e.g., 30"
                            type="number"
                            step="0.01"
                            value={opticalItem.quantity}
                            onChange={(e) => updateOpticalItem(opticalItem.id, 'quantity', e.target.value)}
                            className="border-primary/30 focus:border-primary placeholder:opacity-64"
                            style={{ backgroundColor: '#FFFFFF' }}
                          />
                        </div>
                        ...
                        <div className="space-y-2">
                          <Label className="opacity-72" style={{ color: 'rgba(0, 0, 0, 0.64)' }}>Unit Price (₱) *</Label>
                          <Input
                            placeholder="0.00"
                            type="number"
                            step="0.01"
                            value={opticalItem.unitPrice}
                            onChange={(e) => updateOpticalItem(opticalItem.id, 'unitPrice', e.target.value)}
                            className="border-primary/30 focus:border-primary placeholder:opacity-64"
                            style={{ backgroundColor: '#FFFFFF' }}
                          />
                        </div>
                      ...
                      <div className="mt-4 p-3 bg-secondary/10 rounded-lg">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-semibold text-primary">₱{opticalItem.subtotal.toFixed(2)}</span>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
              ...
              <div className="space-y-2">
                <Label htmlFor="notes" className="opacity-72" style={{ color: 'rgba(0, 0, 0, 0.64)' }}>Additional Notes (Optional)</Label>
                <Textarea
                  id="notes"
                  ...
                  className="border-primary/20 focus:border-primary placeholder:opacity-64"
                  style={{ backgroundColor: '#FFFFFF' }}
                />
              </div>
            </CardContent>
          ...
          <Card className="border-2 border-secondary/20 bg-secondary/5">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <Label className="text-sm text-muted-foreground" style={{ color: 'rgba(0, 0, 0, 0.64)' }}>Employee</Label>
                  <p className="font-medium">John Doe (EMP-2024-001)</p>
                </div>
                <div>
                  <Label className="text-sm text-muted-foreground" style={{ color: 'rgba(0, 0, 0, 0.64)' }}>Department</Label>
                  <p className="font-medium">IT Department</p>
                </div>
                <div>
                  <Label className="text-sm text-muted-foreground" style={{ color: 'rgba(0, 0, 0, 0.64)' }}>Request Date</Label>
                  <p className="font-medium">{new Date().toLocaleDateString()}</p>
                </div>
                <div>
                  <Label className="text-sm text-muted-foreground" style={{ color: 'rgba(0, 0, 0, 0.64)' }}>Total Receipts</Label>
              ...
              {/* Documents Summary */}
              <div className="space-y-2">
                <Label className="text-sm text-muted-foreground" style={{ color: 'rgba(0, 0, 0, 0.64)' }}>Documents Uploaded:</Label>
              ...
              {/* Optical Items Summary */}
              <div className="space-y-2">
                <Label className="text-sm" style={{ color: '#000000' }}>Optical Items Breakdown ({opticalItems.length} items):</Label>
                <div className="bg-white/50 rounded-lg p-3 space-y-2">
                  {opticalItems.map((opticalItem, index) => (
                    opticalItem.name && (
                      <div key={opticalItem.id} className="flex justify-between items-center text-sm">
                        <div className="flex-1">
                          <span className="font-medium">{index + 1}. {opticalItem.name}</span>
                          <span className="text-xs text-muted-foreground ml-2">
                            (Qty: {opticalItem.quantity} × ₱{opticalItem.unitPrice})
                          </span>
                        </div>
                        <span className="font-medium text-right">₱{opticalItem.subtotal.toFixed(2)}</span>
                      </div>
                    )
                  ))}
                </div>
              ...
              {receipts.some(r => r.isPWD) && (
                <>
                  <div className="space-y-2">
                    <Label className="text-sm text-muted-foreground" style={{ color: 'rgba(0, 0, 0, 0.64)' }}>Deductions:</Label>
                    <div className="bg-white/50 rounded-lg p-3 space-y-2">
                      {receipts.map((receipt, index) => (
                        receipt.isPWD && (parseFloat(receipt.vatDeduction) > 0 || parseFloat(receipt.pwdDiscount) > 0) && (
                          <div key={receipt.id} className="space-y-1">
                            <div className="text-sm font-medium text-muted-foreground">Receipt {index + 1}:</div>
                            {parseFloat(receipt.vatDeduction) > 0 && (
                              <div className="flex justify-between items-center text-sm ml-4">
                                <span>VAT Deduction</span>
                                <span className="font-medium text-red-600">- ₱{parseFloat(receipt.vatDeduction).toFixed(2)}</span>
                              </div>
                            )}
                            {parseFloat(receipt.pwdDiscount) > 0 && (
                              <div className="flex justify-between items-center text-sm ml-4">
                                <span>PWD Discount</span>
                                <span className="font-medium text-red-600">- ₱{parseFloat(receipt.pwdDiscount).toFixed(2)}</span>
                              </div>
                            )}
                          </div>
                        )
