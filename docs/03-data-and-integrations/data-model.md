...
HR/Admin and line-manager decisions are modeled as `reimbursement_decisions` records. The current workflow requires line-manager sign-off first; the request remains Pending while `current_review_stage` moves from `line_manager_review` to `hr_admin_review`, then HR/Admin makes the final approve or deny decision.
...
| `reimbursement_decisions` | Line-manager sign-off or decline events and HR/Admin approval or denial events. | Belongs to one request and one actor user. |
...
- STAT-001 through STAT-009
- WF-001 through WF-013
- RPT-001 through RPT-007

Workflow stage behavior:
- A submitted request starts as `status = pending` and `current_review_stage = line_manager_review`.
- Line-manager sign-off keeps `status = pending` and moves `current_review_stage` to `hr_admin_review`.
- HR/Admin approval sets `status = approved`, moves `current_review_stage` to `completed`, and records `final_decided_at`.
- HR/Admin denial sets `status = denied`, moves `current_review_stage` to `completed`, and records `final_decided_at`.
- Line-manager decline sets `status = declined`, moves `current_review_stage` to `completed`, and records `final_decided_at`.

Display label mapping:

| Stored status | Current review stage | User-facing label |
| --- | --- | --- |
...
Records each line-manager and HR/Admin review decision in order.
...
Stage usage:
- `line_manager_review`: line-manager sign-off or decline.
- `hr_admin_review`: HR/Admin final approval or denial.
- `completed`: terminal stage after HR/Admin approval, HR/Admin denial, or line-manager decline.

Decision usage:
- `sign_off`: line manager signs off the request and routes it to HR/Admin review. This is a workflow transition, not final approval.
- `approve`: HR/Admin final approval.
- `deny`: HR/Admin final denial.
- `decline`: line-manager denial before HR/Admin review.
