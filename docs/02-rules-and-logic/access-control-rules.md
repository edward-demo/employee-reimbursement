...
### ACL-007: HR/Admin Review Scope Starts After Line Manager Sign-Off
Admin-role users can see request lists and details, but final approve/deny actions apply only after the request has line-manager sign-off and `current_review_stage = hr_admin_review`.

### ACL-008: Logout Clears Role Session
