...
create type decision_type as enum ('approve', 'deny', 'decline');
create type review_stage as enum ('line_manager', 'admin', 'finance');
...
create table reimbursement_requests (
  ...
  notes text null,
  current_review_stage review_stage null,
...
Stores line-manager, admin, and future finance decisions.
...
Suggested admin denial reason codes:
- `duplicate`
- `incomplete_details`
- `others`
...
- Closed requests should not accept approve, deny, or decline decisions unless a future reopen workflow is defined.
...
- Should `current_review_stage` become mandatory once the line-manager/admin workflow order is finalized?
