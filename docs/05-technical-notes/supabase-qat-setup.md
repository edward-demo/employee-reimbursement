...
- Submitted reimbursement requests start in `current_review_stage = line_manager_review`; line-manager sign-off moves them to `current_review_stage = hr_admin_review` while they remain Pending for HR/Admin review.
...
- Line-manager queues show pending requests in the `line_manager_review` stage. HR/Admin final approve/deny actions happen after line-manager sign-off.
